...
class PositionPaneComponent extends Component {
	...
	handleSell() {
		this.props.selectedOrderSide('Market Sell', { symbol: this.props.stock.instrument.symbol, instrumentUrl: this.props.stock.instrument.url, side: 'sell', activePane: 'sell', hasBackButton: true});
	}

	handleBuy() {
		this.props.selectedOrderSide('Market Buy', { symbol: this.props.stock.instrument.symbol, instrumentUrl: this.props.stock.instrument.url, side: 'buy', activePane:'buy', hasBackButton: true});
	}

	handleMoreNews() {
		// need to make custom action to do this and change primary color
		this.props.showMoreNews(`${this.props.stock.instrument.symbol} News`, {symbol: this.props.stock.instrument.symbol, activePane: 'news', hasBackButton: true});
	}
    ...
    render() {
		// if(this.props.stock.lastUpdated) {
			const { changePrimaryColor, primaryColor, stockType, showPastOrder } = this.props;
										...
										return (
											<div key={index}>
												<ListItem
													...
													onTouchTap={() => showPastOrder('', {activePane: 'order', hasBackButton: true, symbol: instrument.symbol, orderId: order.id})}
												>
